...
options.accelerationAmplitude = [0.06;0.05]*5;
...
resultTimeVarying = run_local_accel_observability_experiment( ...
    dim,makePlots,simulationTime,dt,seed,options);

disp(resultTimeVarying.summary);

fprintf('\nAll-four-pulse, time-varying acceleration comparison\n');
fprintf('  local mean RMSE               : %.8f\n', ...
    resultTimeVarying.pulsePair.postActiveMeanAccelerationRMSE);
fprintf('  equal geometry RMSE           : %.8f\n', ...
    resultTimeVarying.pulsePair.postEqualGeometryAccelerationRMSE);
fprintf('  bounded quality geometry RMSE : %.8f\n', ...
    resultTimeVarying.pulsePair. ...
        postQualityWeightedGeometryAccelerationRMSE);
fprintf('  quality-fusion valid rate     : %.4f\n', ...
    resultTimeVarying.pulsePair.postQualityFusionValidRate);
fprintf('  total maneuver Delta-V        : %.4f\n', ...
    resultTimeVarying.pulsePair.totalDeltaV);
